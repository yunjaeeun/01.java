...
public class MemberService {
    ...
    public void selectAllMembers() {
        ...
        if(!selectedMembers.isEmpty()) {        // 회원이 한명이라도 조회 된다면
            ...
            return;                             // 이후 코드와 상관 없이 메소드 종료(호출한곳으로 돌아감)
        }

        /* 설명. 조건이 맞지 않아 출력을 하는 구문(위의 조건 충족 시 실행되지 않음)(feat.else 안 쓰기) */
...
//        System.out.println("사용자가 입력한 member 확인: " + member);

        int lastNumberNo = mr.selectLastMemberNo();
        member.setMemNo(lastNumberNo + 1);              // autoincrement
    ...
    public void deleteMember(int memNo) {
        ...
        if (result > 0) {
            System.out.println(memNo + "번 회원 탈퇴를 성공하셨습니다.");
            return;
        }
