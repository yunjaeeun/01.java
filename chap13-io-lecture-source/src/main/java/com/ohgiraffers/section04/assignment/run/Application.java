...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        while (true) {              // 사용자가 몇바퀴를 돌릴지 모르니까 무한반복문 작성
            ...
            switch (input) {
                case 1: ms.selectAllMembers();
                    break;
                case 2: ms.selectMember(chooseMemberNo());
                    break;
                case 3: ms.registMember(signUp());
                    break;
                case 4: ms.deleteMember(chooseMemberNo());
                    break;
                case 9:
                    System.out.println("프로그램을 종료합니다.");
                    return;
                default:
                    System.out.println("번호를 재대로 다시 입력해 주세요");
            }
        }
    }
    ...
    private static Member signUp() {
        ...
        for (int i = 0; i < hobbies.length; i++) {
            System.out.print((i + 1) + "번쨰 취미를 입력하세요: ");
            String input = sc.nextLine();
            hobbies[i] = input;
        }
        newInfo = new Member(id, pwd, age, hobbies);        // 생성자를 통해 각각 입력 받을수 있음
        ...
        /* 필기.
         *  회원으로부터 회원가입을 위한 정보를 입력받아 Member 타입 객체 하나로 가공처리할 때 방법이 두 가지가 있다.
         *  1. 생성자 방식(장점: 한줄 코딩 가능, 단점: 따로 생성자 추가)
         *  2. setter 방식(장점: 초기화 할 갯수 수정 용이, 단점: 코드 줄 수 늘어날 수 있음)
        * */
        newInfo.setBloodType(bt);                           // setter를 통해 객체를 입력받을 수 있음.
