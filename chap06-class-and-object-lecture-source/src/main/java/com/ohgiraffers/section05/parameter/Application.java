...
public class Application {
    public static void main(String[] args) {
        ...
        * */
        ...
        int num = 20;
        System.out.println("call by vaule 전: " + num);
        pt.testPrimitiveTypeParameter(num);         // call by value(리터럴 값에 의한 호출)      20이라는 리터럴 값을 넘겨주는거라 변하지 않음
        System.out.println("call by vaule 후: " + num);

        /* 목차. 2. 기본자료형 배열을 매개변수로 전달 받는 메소드 호출 */
        int[] iArr = new int[]{1, 2, 3, 4, 5};
        System.out.println("call by reference 전: " + Arrays.toString(iArr));
        pt.testPrimitiveTypeArrayParameter(iArr);   // call by reference(참조 값에 의한 호출)   주소값이라는 참조 값을 넘겨주기 때문에 원래의 대상도 변함.
